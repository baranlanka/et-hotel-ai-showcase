...
class ModelMappingConfig(BaseSettings):
    ...
    Environment Variables:
        MODEL_EXTRACTION: Model for extraction/processing
        MODEL_VALIDATION: Model for validation
        MODEL_DESCRIPTIONS: Model for hotel descriptions
        MODEL_SUMMARIES: Model for legacy summaries
...
class ValidationConfig(BaseSettings):
    """Pydantic-settings model for validation/refinement.
...
class PipelineExtraConfig(BaseSettings):
    ...
    Env var names:
        ...
        HOTEL_PROMPT_LABEL  — used in et_langgraph/nodes/descriptions_hotel.py
        PROMPT_LABEL        — fallback for HOTEL_PROMPT_LABEL
    """
    ...
    # Both label fields are read; description_prompt_label resolves the
    # HOTEL_PROMPT_LABEL → PROMPT_LABEL → "production" fallback chain.
    hotel_prompt_label: Optional[str] = Field(
        default=None, alias="HOTEL_PROMPT_LABEL"
    )
    prompt_label: Optional[str] = Field(
        default=None, alias="PROMPT_LABEL"
    )
    # A/B override knobs for the room-description and extraction
    # prompts; each falls back to PROMPT_LABEL → "production". Named with the
    # `_env` suffix (like room_brief_prompt_label_env below) so the raw input
    # stays distinct from the resolved room_prompt_label / extraction_prompt_label.
    room_prompt_label_env: Optional[str] = Field(
        default=None, alias="ROOM_PROMPT_LABEL"
    )
    extraction_prompt_label_env: Optional[str] = Field(
        default=None, alias="EXTRACTION_PROMPT_LABEL"
    )
    ...
    def _resolve_prompt_label(self) -> "PipelineExtraConfig":
        """Resolve the specific-label → PROMPT_LABEL → "production" chains."""
        self.description_prompt_label = (
            self.hotel_prompt_label
            or self.prompt_label
            or "production"
        )
        self.room_prompt_label = (
            self.room_prompt_label_env
            or self.prompt_label
            or "production"
        )
        self.extraction_prompt_label = (
            self.extraction_prompt_label_env
            or self.prompt_label
            or "production"
        )
        # Brief label is independent of the label chain (it's a net-new prompt);
...
def get_pipeline_extra_settings() -> PipelineExtraConfig:
    ...
    REVIEW_BATCH_SIZE, HOTEL_PROMPT_LABEL and PROMPT_LABEL are
...
def reset_llm_settings_cache() -> None:
    ...
    cached accessors:
        - DESCRIPTIONS_MAX_TOKENS, VISION_MAX_TOKENS, LLM_ALLOW_EXPENSIVE_MODELS
          → get_llm_settings()
        - REVIEW_BATCH_SIZE, *_PROMPT_LABEL → get_pipeline_extra_settings()
