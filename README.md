...
- **A Temporal-orchestrated, 5-agent cold-outreach engine** — it mines personalization hooks from reviews, opens a conversation, qualifies replies, and drives a funnel *autonomously* — hardened against the OWASP-LLM risks it actually faces (prompt injection, insecure output handling, sensitive-info disclosure, excessive agency, unbounded consumption — **LLM01/02/06/08/10**) with a **deterministic, fail-closed "money-action" gate** and a **reproducible red-team + eval harness**.
